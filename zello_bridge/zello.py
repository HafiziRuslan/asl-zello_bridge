...
class ZelloController:
	def __init__(self, stream_in: AsyncByteStream, stream_out: AsyncByteStream, usrp_ptt: asyncio.Event, zello_ptt: asyncio.Event):
		...
		self._codec_header_b64 = base64.b64encode(struct.pack('<hbb', 8000, 1, 20)).decode('utf8')
	...
	async def authenticate(self, ws):
		...
		if self._logger.isEnabledFor(logging.DEBUG):
			self._logger.debug(f'Sending logon payload: {json.dumps(self._redact(payload))}')
	...
	async def run(self):
		try:
			self._tasks = [asyncio.create_task(self.run_rx()), asyncio.create_task(self.monitor()), asyncio.create_task(self.run_tx())]
			...
			while not self._shutdown:
				...
				if self._token_expiry is not None:
					...
					if time_until_expiry.total_seconds() <= AUTH_TOKEN_EXPIRY_THRESHOLD and not self._txing:
						if self._logger.isEnabledFor(logging.DEBUG):
							self._logger.debug(f'Token expires in {time_until_expiry.total_seconds():.0f}s, reauthenticating')
	...
	async def start_tx(self):
		...
		if self._last_login_at and (now - self._last_login_at).total_seconds() < POST_LOGIN_COOLDOWN_SEC:
			remaining = POST_LOGIN_COOLDOWN_SEC - (now - self._last_login_at).total_seconds()
			self._debug_skip(f'post-login cooldown ({max(0.0, remaining):.2f}s left)')
			return
		...
		try:
			...
			if self._logger.isEnabledFor(logging.DEBUG):
				self._logger.debug(f'Waiting for stream_id assignment (seq={seq_val})...')
			timeout = 2.0
			interval = 0.05
			waited = 0.0
			...
			if self._stream_id is None:
				...
				self._start_retry_after = datetime.now(timezone.utc) + timedelta(seconds=CHANNEL_NOT_READY_BACKOFF_SEC)
				return
			self._txing = True
			self._pkt_id = 0
			self._frame_window_start = None
			...
			if self._logger.isEnabledFor(logging.DEBUG):
				self._logger.debug(f'Started Zello stream {self._stream_id} (seq={seq_val})')
		except Exception as e:
	...
	async def run_tx(self):
		...
		try:
			while not self._shutdown:
				...
				if self._auth_in_progress and self._auth_started_at is not None:
					if (time.monotonic() - self._auth_started_at) > AUTH_WATCHDOG_SEC:
						self._logger.warning('Auth watchdog (TX loop) tripped; clearing auth_in_progress')
						self._auth_in_progress = False
						self._auth_started_at = None
						self._auth_seq = None
				if self._ws is None or self._ws.closed:
				...
				if self._woodpecker_until and now < self._woodpecker_until:
					if not self._in_woodpecker_backoff and self._logger.isEnabledFor(logging.DEBUG):
						self._logger.debug(f'Woodpecker backoff until {self._woodpecker_until}')
				...
				if self._in_woodpecker_backoff and (not self._woodpecker_until or now >= self._woodpecker_until):
					if self._logger.isEnabledFor(logging.DEBUG):
						self._logger.debug('Woodpecker backoff ended, resuming TX attempts')
					self._in_woodpecker_backoff = False
				if self._empty_msg_backoff_until and now < self._empty_msg_backoff_until:
					if not self._in_empty_backoff and self._logger.isEnabledFor(logging.DEBUG):
						self._logger.debug(f'Empty-message backoff until {self._empty_msg_backoff_until}')
				...
				if self._in_empty_backoff and (not self._empty_msg_backoff_until or now >= self._empty_msg_backoff_until):
					if self._logger.isEnabledFor(logging.DEBUG):
						self._logger.debug('Empty-message backoff ended, resuming TX attempts')
					self._in_empty_backoff = False
				try:
					if not self._usrp_ptt.is_set():
						if self._ptt_down_at:
							...
							if self._logger.isEnabledFor(logging.DEBUG):
								self._logger.debug(f'USRP PTT released at {now}')
							self._ptt_down_at = None
						if sending:
							sending = False
							first_pcm_logged = False
						...
						if self._ptt_down_at is None:
							...
							if self._logger.isEnabledFor(logging.DEBUG):
								self._logger.debug(f'USRP PTT down at {self._ptt_down_at}')
					...
					if not first_pcm_logged and self._ptt_down_at:
						if self._logger.isEnabledFor(logging.DEBUG):
							latency_ms = (datetime.now(timezone.utc) - self._ptt_down_at).total_seconds() * 1000.0
							self._logger.debug(f'First PCM after PTT: {latency_ms:.1f} ms (len={len(pcm)})')
					...
					if self._stream_id is not None and isinstance(self._stream_id, int):
						frame = struct.pack('>bii', 1, self._stream_id, self._pkt_id) + opus
						...
						self._logger.warning(f'Invalid stream_id: {self._stream_id}, stopping transmission')
						sending = False
						first_pcm_logged = False
						await self._end_tx()
				except asyncio.TimeoutError:
	...
	async def run_rx(self):
		...
		while not self._shutdown:
			try:
				...
				if self._logger.isEnabledFor(logging.DEBUG):
					self._logger.debug(f'Opening WebSocket to {os.environ.get("ZELLO_WS_ENDPOINT")}')
				async with aiohttp.ClientSession(connector=conn) as session:
					try:
						async with session.ws_connect(os.environ.get('ZELLO_WS_ENDPOINT'), autoping=True, heartbeat=30.0) as ws:
							...
							async for msg in ws:
								...
								if ws.closed:
									...
									if self._logger.isEnabledFor(logging.DEBUG):
										self._logger.debug(f'WebSocket connection closed code={code} uptime={uptime:.1f}s reason={reason}')
										self._logger.debug(
								...
								if msg.type == aiohttp.WSMsgType.ERROR:
									...
									if 'error' in data:
										if data.get('error') == 'kicked':
											...
											if self._woodpecker_until and datetime.now(timezone.utc) < self._woodpecker_until:
												prev = (self._woodpecker_until - datetime.now(timezone.utc)).seconds
												backoff = min(8, prev * 2)
											self._woodpecker_until = datetime.now(timezone.utc) + timedelta(seconds=backoff)
											if self._logger.isEnabledFor(logging.DEBUG):
												self._logger.debug(f'Applying woodpecker backoff until {self._woodpecker_until}')
											if self._txing:
												await self._end_tx()
											continue
											...
											if self._empty_msg_backoff_until and datetime.now(timezone.utc) < self._empty_msg_backoff_until:
												prev = (self._empty_msg_backoff_until - datetime.now(timezone.utc)).seconds
												backoff = min(8, prev * 2)
											else:
												backoff = 1
											self._empty_msg_backoff_until = datetime.now(timezone.utc) + timedelta(seconds=backoff)
											if self._logger.isEnabledFor(logging.DEBUG):
												self._logger.debug(f'Applying empty-message backoff until {self._empty_msg_backoff_until}')
											if self._txing:
												await self._end_tx()
											continue
											...
											self._channel_ready = False
											self._channel_backoff_until = datetime.now(timezone.utc) + timedelta(
												seconds=CHANNEL_NOT_READY_BACKOFF_SEC
											)
											self._start_retry_after = self._channel_backoff_until
											if self._logger.isEnabledFor(logging.DEBUG):
												self._logger.debug(f'Channel backoff until {self._channel_backoff_until}')
											continue
										else:
									...
									if 'command' in data:
										if data['command'] == 'on_stream_start':
											...
											if self._logger.isEnabledFor(logging.DEBUG):
												self._logger.debug(f'Talk user set: {self._talk_user}')
											if user:
											...
											if self._talk_start is not None:
												dur = (now_dt - self._talk_start).total_seconds()
											user = (
												data.get('from')
												or data.get('user')
											...
											if self._channel_ready != desired and self._logger.isEnabledFor(logging.DEBUG):
												self._logger.debug(f"Channel ready -> {desired} (status='{status}')")
									...
									if 'success' in data:
										...
										if self._auth_seq is not None and data.get('seq') == self._auth_seq:
											...
											self._start_retry_after = None
											if self._logger.isEnabledFor(logging.DEBUG):
												self._logger.debug('Authentication success event received (seq match)')
											# token info may come alongside or not
										if 'stream_id' in data:
											self._stream_id = data['stream_id']
											if self._logger.isEnabledFor(logging.DEBUG):
												self._logger.debug(f'Received Zello stream_id: {self._stream_id} (seq={data.get("seq")})')
										if 'refresh_token' in data:
											self._logger.info('Authentication successful!')
											self._refresh_token = data['refresh_token']
											if self._auth_started_at is not None and self._logger.isEnabledFor(logging.DEBUG):
												auth_ms = (time.monotonic() - self._auth_started_at) * 1000.0
												self._logger.debug(f'Auth completed in ~{auth_ms:.0f} ms')
											self._auth_in_progress = False
											self._auth_started_at = None
											self._auth_seq = None
											self._last_login_at = datetime.now(timezone.utc)
											...
											try:
												...
												if exp:
													...
													if self._logger.isEnabledFor(logging.DEBUG):
														self._logger.debug(f'Refresh token expiry set to {self._token_expiry}')
											except Exception as e:
												if self._logger.isEnabledFor(logging.DEBUG):
									...
									if self._logger.isEnabledFor(logging.DEBUG):
										self._logger.debug(f'RX BINARY {len(msg.data)} bytes')
									try:
										data = msg.data[9:]
										pcm = decoder.decode(bytearray(data))
										await self._stream_out.write(pcm)
									except Exception as e:
										self._logger.error(f'Failed to decode audio: {e} bytes={len(msg.data)}')
								else:
						...
						if self._logger.isEnabledFor(logging.DEBUG):
							self._logger.debug('Network hint: DNS/connection issue; will retry in 5s')
					except asyncio.TimeoutError:
